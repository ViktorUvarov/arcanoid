...
window.addEventListener('load', function () {
    ...
    for (let j = 0; j < 8; j++) {
        for (let i = 0; i < 40; i++) {
            enemies.push(SPIN.create_node(30 + (10 + 40) * i, 20 + (10 + 40) * j, 15, 15, '#ADFF2F', enemy_ai));
        }
    }

    let fire = (x, y) => {
        if (SPIN.get_timer() > 0) {
            SPIN.create_node(x, y, 150, 100, '#FF0000', bullet_ai);
            SPIN.clear_timer();
        }
    };

    SPIN.create_node(1920/2-50, 1080-50-60, 40, 40, '#FF4500', (node) => {
        if (SPIN.key('KeyA'))
            node.x -= 10;
        if (SPIN.key('KeyD'))
            node.x += 10;
    ...
    SPIN.set_draw((s) => {
        s.draw_text(1920/2-120, 10, '#8cff00', 'Игровой счет: '+score);
    });
});